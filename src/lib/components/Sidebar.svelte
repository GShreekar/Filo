<script lang="ts">
  import { sidebarCollapsed, folders, notes, selectedNote, selectedFolder, contextMenu, confirmModal, inputModal, searchQuery, searchResults, showSearchResults, selectedSearchIndex } from '$lib/stores';
  import { createFolder, createNote, deleteFolder, deleteNote, updateFolder, updateNote } from '$lib/firebase-service';
...
<div class="h-full bg-gray-50 dark:bg-gray-900 border-r border-gray-200 dark:border-gray-700 flex flex-col">
  <!-- Header -->
  <div class="p-4 border-b border-gray-200 dark:border-gray-700">
    ...
    <div class="relative mb-3">
      ...
      <input
        ...
        class="w-full pl-10 pr-4 py-2 text-sm border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
      />
      {#if isSearching}
        <div class="absolute right-3 top-1/2 transform -translate-y-1/2 text-xs text-gray-500 dark:text-gray-400">
    ...
    <div class="flex gap-2">
      <button
        ...
        class="flex-1 flex items-center justify-center gap-2 px-3 py-2 text-sm bg-green-600 text-white rounded-lg hover:bg-green-700 transition-colors"
      >
        <Plus class="w-4 h-4" />
        New Note
      </button>
      ...
      <button
        ...
        class="flex-1 flex items-center justify-center gap-2 px-3 py-2 text-sm bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors"
      >
        <Plus class="w-4 h-4" />
        New Folder
      </button>
    ...
    {#if isSearching}
      ...
      <div class="p-2">
        ...
        {#each $searchResults as result, index}
          {#if result.type === 'note' && result.noteResult}
            <button
              ...
              class="w-full text-left p-3 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-lg group transition-colors"
            ...
            >
              <div class="flex items-center gap-2 mb-1">
                ...
                <span class="text-sm font-medium text-gray-900 dark:text-gray-100 truncate">
                  {@html highlightText(result.noteResult.note.title, result.noteResult.titleMatches)}
                </span>
                {#if result.noteResult.matchType === 'recent'}
                  <span class="text-xs bg-green-100 dark:bg-green-900 text-green-800 dark:text-green-200 px-1.5 py-0.5 rounded-full">Recent</span>
                {/if}
              </div>
          ...
          {:else if result.type === 'folder' && result.folderResult}
            ...
            >
              <div class="flex items-center gap-2 mb-1">
                ...
                <span class="text-sm font-medium text-gray-900 dark:text-gray-100 truncate">
                  {@html highlightText(result.folderResult.folder.name, result.folderResult.titleMatches)}
                </span>
                <span class="text-xs bg-blue-100 dark:bg-blue-900 text-blue-800 dark:text-blue-200 px-1.5 py-0.5 rounded-full">Folder</span>
              </div>
              <div class="text-xs text-gray-500 dark:text-gray-400 truncate">
        ...
        {#if $searchResults.length === 0}
          <div class="text-sm text-gray-500 dark:text-gray-400 p-3 text-center">
            No results found
          </div>
        {/if}
      </div>
    {:else}
      ...
      <div class="p-2">
        ...
        {#if standaloneNotes.length > 0}
          <div class="mb-4">
            <h3 class="text-sm font-medium text-gray-500 dark:text-gray-400 mb-2 px-2">
              Notes
            </h3>
            {#each standaloneNotes as note}
              ...
              >
                <button
                  ...
                  class="flex items-center gap-2 flex-1 p-2 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-lg"
                ...
                >
                  ...
                  {#if editingNoteId === note.id}
                    <input
                      ...
                      class="text-sm text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded px-1 py-0.5 focus:ring-1 focus:ring-blue-500 focus:border-blue-500 w-full"
                      type="text"
                      placeholder="Enter note title..."
                    />
                  {:else}
                    <span 
                      class="text-sm text-gray-700 dark:text-gray-300 truncate hover:text-blue-600 dark:hover:text-blue-400 transition-colors cursor-pointer"
            ...
            {#if expandedFolders.has(folder.id)}
              <div class="ml-6 mt-1">
                ...
                {#if getNotesInFolder(folder.id).length === 0}
                  <div class="text-xs text-gray-500 dark:text-gray-400 p-2 italic">
                    No notes yet
                  </div>
                {/if}
              </div>
