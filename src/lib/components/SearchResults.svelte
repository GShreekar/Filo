<script lang="ts">
  ...
  import type { CombinedSearchResult, EnhancedSearchResult, FolderSearchResult } from '$lib/search-service';
  ...
  function groupResultsByType(results: CombinedSearchResult[], query: string) {
    ...
    const titleMatches = results.filter(r => r.type === 'note' && r.noteResult?.matchType === 'title');
    const contentMatches = results.filter(r => r.type === 'note' && r.noteResult?.matchType === 'content');
    const bothMatches = results.filter(r => r.type === 'note' && r.noteResult?.matchType === 'both');
    const recentNotes = results.filter(r => r.type === 'note' && r.noteResult?.matchType === 'recent');
  ...
  function getMatchTypeLabel(matchType: string) {
    switch (matchType) {
      ...
      case 'both': return 'Title & content match';
      case 'recent': return 'Recent note';
      case 'folder': return 'Folder match';
      default: return '';
    }
  }
...
<div class="border-t border-gray-200 dark:border-gray-700">
  ...
  {:else}
      ...
      {#if groupedResults.folders.length > 0}
        ...
        {#each groupedResults.folders as result, index}
          {#if result.folderResult}
            <button
              class="w-full text-left p-4 hover:bg-gray-50 dark:hover:bg-gray-800 border-b border-gray-100 dark:border-gray-700 last:border-b-0 transition-colors"
        ...
        {#each groupedResults.bothMatches as result, index}
          {#if result.noteResult}
            ...
            >
              <div class="flex items-start gap-3">
                <div class="flex-shrink-0 mt-1">
                  <svelte:component this={getMatchTypeIcon(result.noteResult.matchType)} size={16} class="text-blue-500 dark:text-blue-400" />
                </div>
                <div class="flex-1 min-w-0">
                  <div class="font-medium text-gray-900 dark:text-gray-100 truncate">
                  ...
                  {/if}
                  <div class="flex items-center justify-between mt-2">
                    <span class="text-xs text-gray-500 dark:text-gray-400">{getMatchInfo(result.noteResult)}</span>
                    <span class="text-xs text-gray-500 dark:text-gray-400">{formatTimeAgo(result.noteResult.lastModified)}</span>
                  </div>
                </div>
              </div>
            </button>
    ...
    <!-- Search tips -->
    {#if results.length > 0}
      <div class="px-4 py-2 text-xs text-gray-500 dark:text-gray-400 bg-gray-50 dark:bg-gray-800 border-t border-gray-200 dark:border-gray-700">
